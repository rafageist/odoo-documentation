...
- `expense_manager_id`: `Many2one` (comodel `res.users`, compute `_compute_expense_manager`, store `True`)
...
- Detected methods: 4
- Action methods: none
- Compute methods: `_compute_expense_manager`
...
hr_employee --> res_users : expense_manager_id
@enduml
```
