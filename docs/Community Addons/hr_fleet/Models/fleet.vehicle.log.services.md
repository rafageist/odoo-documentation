...
- `purchaser_employee_id`: `Many2one` (comodel `hr.employee`, compute `_compute_purchaser_employee_id`, store `True`)

## Method hints

- Detected methods: 2
- Action methods: none
- Compute methods: `_compute_purchaser_employee_id`, `_compute_purchaser_id`
...
fleet_vehicle_log_services --> hr_employee : purchaser_employee_id
@enduml
```
