---
tags: [odoo, community, generated, views]
---

# views/account_move_reversal_view.xml

- Module: [[docs/Community Addons/l10n_tw_edi_ecpay/l10n_tw_edi_ecpay|l10n_tw_edi_ecpay]]
- Scope: Community Addons
- Source file: `views/account_move_reversal_view.xml`
- Views: 1
- Actions: 0
- Menus: 0
- Rules: 0

## View records

### `view_account_move_reversal_inherit_ecpay`
- Name: ec_account_invoice_refund_form_inherit
- Model: `account.move.reversal`
- Type: inferred from arch
- Inherits: `account.view_account_move_reversal`
- Root tag: `xpath`
- Field references: 2
- Sample fields: `l10n_tw_edi_allowance_notify_way`, `l10n_tw_edi_refund_agreement_type`
- XPath or positional patches: 2

## Navigation

- **Parent:** [[docs/Community Addons/l10n_tw_edi_ecpay/Views]]

