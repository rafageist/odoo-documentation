...
- `payment_token_count`: `Integer` (compute `_compute_payment_token_count`)
- `payment_token_ids`: `One2many` (comodel `payment.token`)

## Method hints

- Detected methods: 1
- Action methods: none
- Compute methods: `_compute_payment_token_count`
...
res_partner --|> payment_token : payment_token_ids
@enduml
```
