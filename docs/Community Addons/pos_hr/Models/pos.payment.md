...
- `employee_id`: `Many2one` (comodel `hr.employee`, related `pos_order_id.employee_id`, store `True`)

## Method hints

- Detected methods: 1
- Action methods: none
- Compute methods: `_compute_cashier`
...
pos_payment --> hr_employee : employee_id
@enduml
```
