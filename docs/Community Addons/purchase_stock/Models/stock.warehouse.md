...
- `buy_pull_id`: `Many2one` (comodel `stock.rule`)
- `buy_to_resupply`: `Boolean` (comodel `Buy to Resupply`, compute `_compute_buy_to_resupply`)

## Method hints

- Detected methods: 8
- Action methods: none
- Compute methods: `_compute_buy_to_resupply`
...
stock_warehouse --> stock_rule : buy_pull_id
@enduml
```
