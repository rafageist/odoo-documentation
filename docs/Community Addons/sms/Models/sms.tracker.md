...
- Source files: `models/sms_tracker.py`
- Python classes: `SmsTracker`
...
- `mail_notification_id`: `Many2one` (comodel `mail.notification`)
- `sms_uuid`: `Char` (comodel `SMS uuid`)
...
title sms.tracker - Direct Relations
class "sms.tracker" as sms_tracker
class "mail.notification" as mail_notification
sms_tracker --> mail_notification : mail_notification_id
@enduml
```
