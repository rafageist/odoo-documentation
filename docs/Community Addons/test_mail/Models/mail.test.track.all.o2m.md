...
- Python classes: `MailTestTrackAllO2m`
- Description: Sub-model: pseudo tags for tracking
...
- `mail_track_all_id`: `Many2one` (comodel `mail.test.track.all`)
...
title mail.test.track.all.o2m - Direct Relations
class "mail.test.track.all.o2m" as mail_test_track_all_o2m
class "mail.test.track.all" as mail_test_track_all
mail_test_track_all_o2m --> mail_test_track_all : mail_track_all_id
@enduml
```
