---
tags: [odoo, community, generated, views]
---

# views/event_event_views.xml

- Module: [[docs/Community Addons/website_event_exhibitor/website_event_exhibitor|website_event_exhibitor]]
- Scope: Community Addons
- Source file: `views/event_event_views.xml`
- Views: 2
- Actions: 0
- Menus: 0
- Rules: 0

## View records

### `event_event_view_list`
- Name: event.event.view.list.inherit.exhibitor
- Model: `event.event`
- Type: inferred from arch
- Inherits: `event.view_event_tree`
- Root tag: `field`
- Field references: 2
- Sample fields: `sponsor_count`, `stage_id`
- XPath or positional patches: 0

### `event_event_view_form`
- Name: event.event.view.form.inherit.exhibitor
- Model: `event.event`
- Type: inferred from arch
- Inherits: `website_event.event_event_view_form`
- Root tag: `field`
- Field references: 3
- Sample fields: `exhibitor_menu`, `sponsor_count`, `website_url`
- Buttons: `%(event_sponsor_action_from_event)d`
- XPath or positional patches: 1

## Navigation

- **Parent:** [[docs/Community Addons/website_event_exhibitor/Views]]

