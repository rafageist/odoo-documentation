...
- `event_track_wishlisted_count`: `Integer` (compute `_compute_event_track_wishlisted_ids`)
- `event_track_wishlisted_ids`: `Many2many` (comodel `event.track`, compute `_compute_event_track_wishlisted_ids`)

## Method hints

- Detected methods: 4
- Action methods: none
- Compute methods: `_compute_event_track_wishlisted_ids`
...
website_visitor --|> event_track_visitor : event_track_visitor_ids
website_visitor .. event_track : event_track_wishlisted_ids
@enduml
```
