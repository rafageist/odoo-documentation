...
- `is_pending_chat_request`: `Boolean` (comodel `When created from an operator, whether the channel is yet to be opened on the visitor side.`)
- `livechat_visitor_id`: `Many2one` (comodel `website.visitor`)
...
discuss_channel --> website_visitor : livechat_visitor_id
@enduml
```
