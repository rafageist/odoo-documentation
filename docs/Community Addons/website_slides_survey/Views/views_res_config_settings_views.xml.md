---
tags: [odoo, community, generated, views]
---

# views/res_config_settings_views.xml

- Module: [[docs/Community Addons/website_slides_survey/website_slides_survey|website_slides_survey]]
- Scope: Community Addons
- Source file: `views/res_config_settings_views.xml`
- Views: 1
- Actions: 0
- Menus: 0
- Rules: 0

## View records

### `res_config_settings_view_form`
- Name: res.config.settings.view.form.inherit.website.slides.survey
- Model: `res.config.settings`
- Type: inferred from arch
- Inherits: `website_slides.res_config_settings_view_form`
- Root tag: `xpath`
- Field references: 0
- Buttons: `%(website_slides_survey.survey_survey_action_slides)d`
- XPath or positional patches: 1

## Navigation

- **Parent:** [[docs/Community Addons/website_slides_survey/Views]]

