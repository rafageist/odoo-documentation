<!-- GENERATED:VIEWFILE -->
---
tags: [odoo, enterprise, generated, views]
---

# views/account_accountant_menuitems.xml

- Module: [[docs/Enterprise Addons/account_accountant/account_accountant|account_accountant]]
- Scope: Enterprise Addons
- Source file: `views/account_accountant_menuitems.xml`
- Views: 0
- Actions: 0
- Menus: 4
- Rules: 0
## Menus

- `menu_account_fiscal_year`: unnamed
- `menu_account_group`: unnamed
- `account_tag_menu`: unnamed
- `menu_account_reconcile`: Reconcile

## Navigation

- **Parent:** [[docs/Enterprise Addons/account_accountant/Views]]

<!-- GENERATED:VIEWFILE -->
