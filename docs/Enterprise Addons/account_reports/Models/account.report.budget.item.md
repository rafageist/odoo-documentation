...
- Python classes: `AccountReportBudgetItem`
- Description: Accounting Report Budget Item
...
- `amount`: `Float`
- `budget_id`: `Many2one` (comodel `account.report.budget`)
- `date`: `Date`
...
title account.report.budget.item - Direct Relations
...
account_report_budget_item --> account_report_budget : budget_id
account_report_budget_item --> account_account : account_id
@enduml
```
