<!-- GENERATED:FRONTEND -->
---
tags: [odoo, enterprise, generated, frontend]
---

# static/src/web/activity_list_popover_item_patch.xml

- Module: [[docs/Enterprise Addons/approvals/approvals|approvals]]
- Scope: Enterprise Addons
- Asset kind: `xml`
- Source file: `static/src/web/activity_list_popover_item_patch.xml`

## Detected frontend signals

- Component classes: none
- Registry entries: none
- Templates: none

## Navigation

- **Parent:** [[docs/Enterprise Addons/approvals/Frontend]]

<!-- GENERATED:FRONTEND -->
