<!-- GENERATED:FRONTEND -->
---
tags: [odoo, enterprise, generated, frontend]
---

# static/src/owl/components/pdf_group_name/pdf_group_name.js

- Module: [[docs/Enterprise Addons/documents/documents|documents]]
- Scope: Enterprise Addons
- Asset kind: `component`
- Source file: `static/src/owl/components/pdf_group_name/pdf_group_name.js`

## Detected frontend signals

- Component classes: `PdfGroupName`
- Registry entries: none
- Templates: none

## Navigation

- **Parent:** [[docs/Enterprise Addons/documents/Frontend]]

<!-- GENERATED:FRONTEND -->
