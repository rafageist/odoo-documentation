<!-- GENERATED:FRONTEND -->
---
tags: [odoo, enterprise, generated, frontend]
---

# static/src/views/activity/documents_activity_renderer.js

- Module: [[docs/Enterprise Addons/documents/documents|documents]]
- Scope: Enterprise Addons
- Asset kind: `view`
- Source file: `static/src/views/activity/documents_activity_renderer.js`

## Detected frontend signals

- Component classes: `DocumentsActivityRenderer`
- Registry entries: none
- Templates: none

## Navigation

- **Parent:** [[docs/Enterprise Addons/documents/Frontend]]

<!-- GENERATED:FRONTEND -->
