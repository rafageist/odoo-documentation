<!-- GENERATED:FRONTEND -->
---
tags: [odoo, enterprise, generated, frontend]
---

# static/src/views/kanban/documents_kanban_model.js

- Module: [[docs/Enterprise Addons/documents/documents|documents]]
- Scope: Enterprise Addons
- Asset kind: `view`
- Source file: `static/src/views/kanban/documents_kanban_model.js`

## Detected frontend signals

- Component classes: `DocumentsKanbanModel`, `DocumentsKanbanRecord`
- Registry entries: none
- Templates: none

## Navigation

- **Parent:** [[docs/Enterprise Addons/documents/Frontend]]

<!-- GENERATED:FRONTEND -->
