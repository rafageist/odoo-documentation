...
- `documents_hr_payslips_tags`: `Many2many` (comodel `documents.tag`)
- `worker_payroll_folder_id`: `Many2one` (comodel `documents.document`)
...
res_company .. documents_tag : documents_hr_payslips_tags
res_company --> documents_document : worker_payroll_folder_id
@enduml
```
