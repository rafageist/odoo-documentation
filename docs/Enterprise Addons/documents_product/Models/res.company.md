...
- `product_folder_id`: `Many2one` (comodel `documents.document`, compute `_compute_product_folder_id`, store `True`)
- `product_tag_ids`: `Many2many` (comodel `documents.tag`)

## Method hints

- Detected methods: 2
- Action methods: none
- Compute methods: `_compute_product_folder_id`
...
res_company --> documents_document : product_folder_id
res_company .. documents_tag : product_tag_ids
@enduml
```
