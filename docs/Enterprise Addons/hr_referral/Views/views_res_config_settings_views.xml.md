---
tags: [odoo, enterprise, generated, views]
---

# views/res_config_settings_views.xml

- Module: [[docs/Enterprise Addons/hr_referral/hr_referral|hr_referral]]
- Scope: Enterprise Addons
- Source file: `views/res_config_settings_views.xml`
- Views: 1
- Actions: 1
- Menus: 1
- Rules: 0

## View records

### `res_config_settings_view__form`
- Name: unnamed
- Model: `res.config.settings`
- Type: inferred from arch
- Inherits: `base.res_config_settings_view_form`
- Root tag: `xpath`
- Field references: 1
- Sample fields: `hr_referral_background`
- Buttons: `restore_default_referral_background`
- XPath or positional patches: 1

## Actions

- `action_hr_referral_configuration`: `act_window` Settings

## Menus

- `hr_referral_menu_configuration`: Settings

## Navigation

- **Parent:** [[docs/Enterprise Addons/hr_referral/Views]]

