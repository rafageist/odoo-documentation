<!-- GENERATED:FRONTEND -->
---
tags: [odoo, enterprise, generated, frontend]
---

# static/src/js/tours/industry_fsm_sale_report_tour.js

- Module: [[docs/Enterprise Addons/industry_fsm_sale_report/industry_fsm_sale_report|industry_fsm_sale_report]]
- Scope: Enterprise Addons
- Asset kind: `js`
- Source file: `static/src/js/tours/industry_fsm_sale_report_tour.js`

## Detected frontend signals

- Component classes: none
- Registry entries: none
- Templates: none

## Navigation

- **Parent:** [[docs/Enterprise Addons/industry_fsm_sale_report/Frontend]]

<!-- GENERATED:FRONTEND -->
