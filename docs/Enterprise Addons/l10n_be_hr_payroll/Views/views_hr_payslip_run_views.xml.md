<!-- GENERATED:VIEWFILE -->
---
tags: [odoo, enterprise, generated, views]
---

# views/hr_payslip_run_views.xml

- Module: [[docs/Enterprise Addons/l10n_be_hr_payroll/l10n_be_hr_payroll|l10n_be_hr_payroll]]
- Scope: Enterprise Addons
- Source file: `views/hr_payslip_run_views.xml`
- Views: 1
- Actions: 0
- Menus: 0
- Rules: 0

## View records

### `hr_payslip_run_view_kanban`
- Name: hr.payslip.run.view.kanban.inherit.l10n.be.hr.payroll
- Model: `hr.payslip.run`
- Type: inferred from arch
- Inherits: `hr_payroll.hr_payslip_run_view_kanban`
- Root tag: `templates`
- Field references: 1
- Sample fields: `l10n_be_display_eco_voucher_button`
- Buttons: `action_draft`, `action_l10n_be_eco_vouchers`
- XPath or positional patches: 1

## Navigation

- **Parent:** [[docs/Enterprise Addons/l10n_be_hr_payroll/Views]]

<!-- GENERATED:VIEWFILE -->
