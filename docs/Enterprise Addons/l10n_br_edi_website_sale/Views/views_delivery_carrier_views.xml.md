<!-- GENERATED:VIEWFILE -->
---
tags: [odoo, enterprise, generated, views]
---

# views/delivery_carrier_views.xml

- Module: [[docs/Enterprise Addons/l10n_br_edi_website_sale/l10n_br_edi_website_sale|l10n_br_edi_website_sale]]
- Scope: Enterprise Addons
- Source file: `views/delivery_carrier_views.xml`
- Views: 1
- Actions: 0
- Menus: 0
- Rules: 0

## View records

### `view_delivery_carrier_form`
- Name: delivery.carrier.form.inherit.l10n_br_edi.website.sale
- Model: `delivery.carrier`
- Type: inferred from arch
- Inherits: `delivery.view_delivery_carrier_form`
- Root tag: `group`
- Field references: 2
- Sample fields: `l10n_br_edi_freight_model`, `l10n_br_edi_transporter_id`
- XPath or positional patches: 1

## Navigation

- **Parent:** [[docs/Enterprise Addons/l10n_br_edi_website_sale/Views]]

<!-- GENERATED:VIEWFILE -->
