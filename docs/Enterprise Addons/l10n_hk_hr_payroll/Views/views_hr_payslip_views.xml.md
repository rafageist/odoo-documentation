---
tags: [odoo, enterprise, generated, views]
---

# views/hr_payslip_views.xml

- Module: [[docs/Enterprise Addons/l10n_hk_hr_payroll/l10n_hk_hr_payroll|l10n_hk_hr_payroll]]
- Scope: Enterprise Addons
- Source file: `views/hr_payslip_views.xml`
- Views: 1
- Actions: 0
- Menus: 0
- Rules: 0

## View records

### `view_hr_payslip_form`
- Name: hr.payslip.form.inherit.l10n_hk_hr_payroll
- Model: `hr.payslip`
- Type: inferred from arch
- Inherits: `hr_payroll.view_hr_payslip_form`
- Root tag: `field`
- Field references: 6
- Sample fields: `currency_id`, `is_superuser`, `l10n_hk_average_daily_wage`, `l10n_hk_includes_eoy_pay`, `l10n_hk_leave_id`, `l10n_hk_worked_days_leaves_count`
- XPath or positional patches: 3

## Navigation

- **Parent:** [[docs/Enterprise Addons/l10n_hk_hr_payroll/Views]]

