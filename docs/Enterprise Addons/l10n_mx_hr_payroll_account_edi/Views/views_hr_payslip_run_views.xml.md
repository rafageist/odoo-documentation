---
tags: [odoo, enterprise, generated, views]
---

# views/hr_payslip_run_views.xml

- Module: [[docs/Enterprise Addons/l10n_mx_hr_payroll_account_edi/l10n_mx_hr_payroll_account_edi|l10n_mx_hr_payroll_account_edi]]
- Scope: Enterprise Addons
- Source file: `views/hr_payslip_run_views.xml`
- Views: 1
- Actions: 0
- Menus: 0
- Rules: 0

## View records

### `l10n_mx_hr_payslip_run_view_kanban`
- Name: l10n.mx.hr.payslip.run.inherit.kanban
- Model: `hr.payslip.run`
- Type: inferred from arch
- Inherits: `hr_payroll_account.hr_payslip_run_view_kanban`
- Root tag: `button`
- Field references: 0
- Buttons: `action_confirm`, `action_generate_cfdi`
- XPath or positional patches: 0

## Navigation

- **Parent:** [[docs/Enterprise Addons/l10n_mx_hr_payroll_account_edi/Views]]

