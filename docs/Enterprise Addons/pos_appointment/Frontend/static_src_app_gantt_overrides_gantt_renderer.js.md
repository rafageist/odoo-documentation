<!-- GENERATED:FRONTEND -->
---
tags: [odoo, enterprise, generated, frontend]
---

# static/src/app/gantt_overrides/gantt_renderer.js

- Module: [[docs/Enterprise Addons/pos_appointment/pos_appointment|pos_appointment]]
- Scope: Enterprise Addons
- Asset kind: `js`
- Source file: `static/src/app/gantt_overrides/gantt_renderer.js`

## Detected frontend signals

- Component classes: `POSAppointmentBookingGanttRenderer`
- Registry entries: none
- Templates: none

## Navigation

- **Parent:** [[docs/Enterprise Addons/pos_appointment/Frontend]]

<!-- GENERATED:FRONTEND -->
