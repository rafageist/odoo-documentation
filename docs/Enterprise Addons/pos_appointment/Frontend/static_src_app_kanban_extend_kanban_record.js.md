<!-- GENERATED:FRONTEND -->
---
tags: [odoo, enterprise, generated, frontend]
---

# static/src/app/kanban_extend/kanban_record.js

- Module: [[docs/Enterprise Addons/pos_appointment/pos_appointment|pos_appointment]]
- Scope: Enterprise Addons
- Asset kind: `js`
- Source file: `static/src/app/kanban_extend/kanban_record.js`

## Detected frontend signals

- Component classes: `PosAppointmentKanbanPopover`, `PosKanbanRecord`
- Registry entries: none
- Templates: none

## Navigation

- **Parent:** [[docs/Enterprise Addons/pos_appointment/Frontend]]

<!-- GENERATED:FRONTEND -->
