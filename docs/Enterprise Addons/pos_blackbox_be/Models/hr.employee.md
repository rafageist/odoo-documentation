...
- `clocked_session_ids`: `Many2many` (comodel `pos.session`)
- `insz_or_bis_number`: `Char` (comodel `National Register Number`, compute `_compute_insz_or_bis_number`, store `True`)

## Method hints

- Detected methods: 3
- Action methods: none
- Compute methods: `_compute_insz_or_bis_number`
...
hr_employee .. pos_session : clocked_session_ids
@enduml
```
