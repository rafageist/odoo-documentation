<!-- GENERATED:VIEWFILE -->
---
tags: [odoo, enterprise, generated, views]
---

# views/chatbot_script_step_views.xml

- Module: [[docs/Enterprise Addons/website_helpdesk_livechat/website_helpdesk_livechat|website_helpdesk_livechat]]
- Scope: Enterprise Addons
- Source file: `views/chatbot_script_step_views.xml`
- Views: 1
- Actions: 0
- Menus: 0
- Rules: 0

## View records

### `chatbot_script_step_view_form`
- Name: chatbot.script.step.view.form.inherit.helpdesk.livechat
- Model: `chatbot.script.step`
- Type: inferred from arch
- Inherits: `im_livechat.chatbot_script_step_view_form`
- Root tag: `field`
- Field references: 2
- Sample fields: `helpdesk_team_id`, `step_type`
- XPath or positional patches: 0

## Navigation

- **Parent:** [[docs/Enterprise Addons/website_helpdesk_livechat/Views]]

<!-- GENERATED:VIEWFILE -->
