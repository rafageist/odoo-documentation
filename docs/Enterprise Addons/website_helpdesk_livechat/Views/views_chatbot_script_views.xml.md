<!-- GENERATED:VIEWFILE -->
---
tags: [odoo, enterprise, generated, views]
---

# views/chatbot_script_views.xml

- Module: [[docs/Enterprise Addons/website_helpdesk_livechat/website_helpdesk_livechat|website_helpdesk_livechat]]
- Scope: Enterprise Addons
- Source file: `views/chatbot_script_views.xml`
- Views: 1
- Actions: 0
- Menus: 0
- Rules: 0

## View records

### `chatbot_script_view_form`
- Name: chatbot.script.view.form.inherit.helpdesk.livechat
- Model: `chatbot.script`
- Type: inferred from arch
- Inherits: `im_livechat.chatbot_script_view_form`
- Root tag: `div`
- Field references: 1
- Sample fields: `ticket_count`
- Buttons: `action_view_tickets`
- XPath or positional patches: 1

## Navigation

- **Parent:** [[docs/Enterprise Addons/website_helpdesk_livechat/Views]]

<!-- GENERATED:VIEWFILE -->
