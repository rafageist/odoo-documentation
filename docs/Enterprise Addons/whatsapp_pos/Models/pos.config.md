...
- `invoice_template_id`: `Many2one` (comodel `whatsapp.template`)
- `receipt_template_id`: `Many2one` (comodel `whatsapp.template`)
- `whatsapp_enabled`: `Boolean` (comodel `WhatsApp Enabled`)
...
pos_config --> whatsapp_template : receipt_template_id
pos_config --> whatsapp_template : invoice_template_id
@enduml
```
